    ...
    public function modify_sidebar_header($params)
    {
        ...
        if( $this->current_sidebar !== 'footer-widget-1' && $this->current_sidebar !== 'footer-widget-2' && $this->current_sidebar !== 'footer-widget-3' && $this->current_sidebar !== 'footer-widget-4' )
        {
            if ( isset( $wp_registered_widgets[ $id ]['callback'][0] ) && is_object( $wp_registered_widgets[ $id ]['callback'][0] ) )
            {
	            ...
	            $additional_class = '';
	            if (isset($instance['widget_class'])) $additional_class .= $instance['widget_class'];
	            if (isset($instance['widget_boxed']) && $instance['widget_boxed']) $additional_class .= ' jeg_pb_boxed';
	            if (isset($instance['widget_shadow']) && $instance['widget_shadow']) $additional_class .= ' jeg_pb_boxed_shadow';
	            if ( ! empty($additional_class) ) $params[0]['before_widget'] = preg_replace('/widget/', 'widget ' . $additional_class, $params[0]['before_widget'], 1);
            }
        }
