...
if(isset($enable_bottom_ribon) && $enable_bottom_ribon) {
...
if((isset($enable_overlay) && $enable_overlay) || (isset($top_ribon_bg) && $top_ribon_bg) || (isset($bottom_ribon_bg) && $bottom_ribon_bg)) {
...
if(isset($background_use_featured) && $background_use_featured) {
    if(is_singular('post'))
    {
        $post_id            = get_the_ID();
        $post_thumbnail_id  = get_post_thumbnail_id( $post_id );
        $image              = wp_get_attachment_image_src($post_thumbnail_id, 'full');
        $styles[] = "background-image: url({$image[0]}) !important";
    } else {
        $image = JNEWS_THEME_URL . '/assets/img/placeholder.jpg';
        $styles[] = "background-image: url({$image}) !important";
    }
}

if(isset($background_repeat) && $background_repeat) {
    $styles[] = "background-repeat: {$background_repeat} !important";
}

if(isset($background_position) && $background_position) {
    $styles[] = "background-position: {$background_position} !important";
}

if(isset($background_attachment) && $background_attachment) {
