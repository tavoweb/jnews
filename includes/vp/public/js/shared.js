...
vp.wp_ext2type = function ( ext ) {
	var ext2type = {
		image       : ['jpg', 'jpeg', 'bmp',  'gif',  'png', 'ico'],
...
if ( vp_wp.use_new_media_upload )
{
	...
	vp.upload_callback = function(e) {
        ...
        wp.media.editor.send.link = function(embed) {
            var result =  wp.media.post( 'vp_send_link_to_editor', {
...
vp.image_upload_callback = function(e) {
	var $this    			= jQuery(this),
	    $input   			= $this.parent('.buttons').prev('input'),
	    $preview 			= $this.parent('.buttons').siblings('.image').find('img'),
...
jQuery(document).on('click', '.vp-js-imageupload', vp.image_upload_callback);


// Multi Image
vp.init_multiimage = function($elements)
{
    ...
    $elements.each(function() {
        var $this    			= jQuery(this),
            $input   			= $this.parent('.buttons').prev('input'),
            $preview 			= $this.parent('.buttons').siblings('.gallery'),
            custom_uploader 	= null,
            attachment			= null;
        ...
        $preview.on('click', 'span.close', function(e){
        ...
        $this.on('click', function() {
            ...
            custom_uploader.on('select', function() {
                ...
                attachment.each( function(model) {
                    ...
                    $preview.append('<span data-id="' + model.id + '" title="' + model.attributes.title + '"><img src="' + thumbnail + '" alt="" /><span class="close"></span></span>');
                    $preview.trigger('change');
                });
            });
...
vp.init_controls = function($parent)
{
	...
	// vp.init_select2($parent.find('.vp-js-select2')); // disable select2
    ...
    vp.init_multiimage($parent.find('.vp-js-multiimage'));
};
