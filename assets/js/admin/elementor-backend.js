(function($){
    ...
    window.open_control = function(control)
    {
        ...
        else if ( wrapper.hasClass('type-multiselect') )
        {
            multiselect_control(wrapper);
        }
    ...
    function multiselect_control(element)
    {
    ...
    function multiauthor_control(element)
    {
        if (element.hasClass('jeg-ajax-load'))
        {
            ...
            if ( value !== '' )
            {
                ajax_control(ajaxurl, value, 'elementor_get_author').done(function(data) {
                    selectize_control(element, data, true, 'vp_find_ajax_author');
                });
            } else {
                selectize_control(element, {}, true, 'vp_find_ajax_author');
            }
        } else {
            selectize_control(element, JSON.parse(element.find('.data-option').text()), false, false);
        }
    }
    ...
    function multitag_control(element)
    {
        if (element.hasClass('jeg-ajax-load'))
        {
            ...
            if ( value !== '' )
            {
                ajax_control(ajaxurl, value, 'elementor_get_tag').done(function(data) {
                    selectize_control(element, data, true, 'vp_find_ajax_post_tag');
                });
            } else {
                selectize_control(element, {}, true, 'vp_find_ajax_post_tag');
            }
        } else {
            selectize_control(element, JSON.parse(element.find('.data-option').text()), false, false);
        }
    ...
    function multicategory_control(element)
    {
        if (element.hasClass('jeg-ajax-load'))
        {
            ...
            if ( value !== '' )
            {
                ajax_control(ajaxurl, value, 'elementor_get_category').done(function(data) {
                    selectize_control(element, data, true, 'vp_find_ajax_post_category');
                });
            } else {
                selectize_control(element, {}, true, 'vp_find_ajax_post_category');
            }
        } else {
            selectize_control(element, JSON.parse(element.find('.data-option').text()), false, false);
        }
    ...
    function selectize_control(element, options, ajax_load, action)
    {
        element.find('input.input-sortable').selectize({
            plugins: ['drag_drop', 'remove_button'],
            options: options,
